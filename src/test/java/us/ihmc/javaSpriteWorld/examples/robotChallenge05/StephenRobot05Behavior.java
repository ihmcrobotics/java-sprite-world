...
public class StephenRobot05Behavior implements Robot05Behavior
{
   ...
   // robot state
   private double xPosition = initialX;
   private double yPosition = initialY;
   ...
   private final double flagAttractionForceMagnitude = 1.0;
   ...
   private final double proximityNearWallToIgnoreFood = 0.8; // doesn't work, dead reckoning not working
   ...
   {
      calculateXYVelocity();
      xPosition += xyVelocity.getX() * dt;
      yPosition += xyVelocity.getY() * dt;
   ...
   private void updateAreaToExplore()
   {
      ...
      else if (areaToExplore.distance(new Point2D(xPosition, yPosition)) < proximityToExploreAreaToSwitch)
      {
         switchAreaToExplore();
         counter = 0;
      }
   ...
   private void bodyFrameToWorldFrame(Tuple2DReadOnly bodyFrameVector, Tuple2DBasics worldFrameVectorToSet)
   {
      ...
      worldFrameVectorToSet.setX(relativeWorldVectorX + xPosition);
      worldFrameVectorToSet.setY(relativeWorldVectorY + yPosition);
   }

   private void worldFrameToBodyFrame(Tuple2DReadOnly worldFrameVector, Tuple2DBasics bodyFrameVectorToSet)
   {
      double dx = worldFrameVector.getX() - xPosition;
      double dy = worldFrameVector.getY() - yPosition;
   ...
   {
      return inDeliverFlagMode && (xPosition > 8.01 && yPosition > 8.01);
   }
   ...
   static void testBodyWorldTransforms()
   {
      ...
      for (int i = 0; i < 100; i++)
      {
         ...
         behavior.xPosition = random.nextDouble() * 5.0;
         behavior.yPosition = random.nextDouble() * 5.0;
