...
public class Sprite
{
   ...
   public void getAxisAlignedBoundingBox(AxisAlignedBoundingBox2D axisAlignedBoundingBoxToPack)
   {
      ...
      if (defaultCollisionPolygons == null) return;
   ...
   public synchronized int addCostume(String filename)
   {
      ...
      if (costume != null) return addCostume(costume);
   ...
   public synchronized int addCostume(SpriteCostume spriteCostume)
   {
      ...
      if (costumeNumber == 0) this.costumeNumber = 0;
   ...
   {
      if (defaultCollisionPolygons == null) return isClickPointInsideSimple(worldX, worldY);
 
      if (collisionPolygonsDirty) computeTransformedCollisionPolygonsAndBoundingBox();
      
      if (!transformedBoundingBox.isPointInside(worldX, worldY)) return false;
      for (int i=0; i<transformedCollisionPolygons.size(); i++)
      {
         ConvexPolygon convexPolygon = transformedCollisionPolygons.get(i);
         if (convexPolygon.isPointInsideBoundingBox(worldX, worldY)) return true;
      }
   ...
   private boolean isClickPointInsideSimple(double worldX, double worldY)
   {
      ...
      if ((worldX > minXToCheck) && (worldX < maxXToCheck) && 
            (worldY > minYToCheck) && (worldY < maxYToCheck))
      {
         return true;
      }
